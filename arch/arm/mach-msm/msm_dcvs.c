	...
	case MSM_DCVS_IDLE_ENTER:
		...
		if (ret < 0 && ret != -13)
			__err("Error (%d) sending idle enter for %s\n",
					ret, core->core_name);
#if 0
 		trace_msm_dcvs_idle("idle_enter_exit", core->core_name, 1);		 		trace_msm_dcvs_idle("idle_enter_exit", core->core_name, 1);
#endif
		break;
...
#if 0
 		trace_msm_dcvs_idle("idle_enter_exit", core->core_name, 0);		 		trace_msm_dcvs_idle("idle_enter_exit", core->core_name, 0);
 		trace_msm_dcvs_iowait("iowait", core->core_name, iowaited);		 		trace_msm_dcvs_iowait("iowait", core->core_name, iowaited);
 		trace_msm_dcvs_slack_time("slack_timer_dcvs", core->core_name,		 		trace_msm_dcvs_slack_time("slack_timer_dcvs", core->core_name,
 							timer_interval_us);		 							timer_interval_us);
#endif
		break;
	}
